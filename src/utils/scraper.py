...
def scrape_race_page(url):
    ...
    try:
        ...
        weather_elem = soup.select_one('.Weather, .RaceData01 .Item01, .RaceData .Item01')  
        track_condition_elem = soup.select_one('.Baba, .RaceData01 .Item03, .RaceData .Item03')

        if weather_elem:
            weather = weather_elem.text.strip()
        if track_condition_elem:
            track_condition = track_condition_elem.text.strip()
        ...
        for i, row in enumerate(horse_rows):
            ...
            try:
                ...
                horse_name = "不明"
                # まず元のセレクタを試す
                horse_name_td = row.select_one('td.Horse_Name, .HorseInfo a')
                if horse_name_td:
                    horse_name = horse_name_td.text.strip()
                else:
                    # 別のセレクタも試してみる
                    horse_name_elem = row.select_one('td:nth-child(4) a, td:nth-child(4) span')
                    if horse_name_elem:
                        horse_name = horse_name_elem.text.strip()
                    elif len(tds) > 3:  # 4番目の要素が存在するか確認
                        horse_name = tds[3].text.strip()  # 4番目の列から直接取得

                # デバッグ用の出力を追加
                if horse_name == "不明":
                    logger.info(f"馬{i+1}の行のHTMLから馬名を抽出できませんでした: {row}")
